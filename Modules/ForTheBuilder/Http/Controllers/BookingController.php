...
class BookingController extends Controller
{
    ...
    {
        $user = Auth::user();
        $models = Booking::where('user_id', $user->id)->get();
        $response = [];
        if (!empty($models)) {
    ...
    public function store(Request $request)
    {
        ...
        if ($booking) {
            ...
            try {
                ...
                $model->expire_dates = json_encode([['comment' => '', 'date' => date('Y-m-d H:i:s', strtotime('+5 days'))]]);
    ...
    public function extend($booking_id, $notification_id)
    {
        ...
        $new_expire_date = [['comment' => '', 'date' => date('Y-m-d H:i:s', strtotime('+5 days'))]];
